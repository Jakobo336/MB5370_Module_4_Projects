...
```{r}
nrow sets the number of rows of facet panels, ncol sets the number of columns. Other options like scales, as.table, dir, and strip.position control whether axes are fixed or free, how panels are filled, and where facet labels appear.
```
...
```{r}
#1. For a line chart -> geom_line()
#For a boxplot -> geom_boxplot()
#For a Historgram -> geom_histogram()
#For a an area chart -> geom_area()
...
-They will look the same, both graphs have data and mappings so they will be identical.
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point() + 
  geom_smooth()
